<x-guest-layout>
    <div class="text-center mb-fluid-8">
        <h2 class="text-fluid-2xl font-black text-gray-900 dark:text-white tracking-tight">{{ __('Choose a New Password') }}</h2>
        <p class="text-fluid-sm text-gray-500 dark:text-gray-400 mt-2 font-medium">{{ __('Make sure it\'s strong and secure') }}</p>
    </div>

    <form method="POST" action="{{ route('password.store') }}" class="space-y-fluid-6">
        @csrf
